...
const requestBatchNFTOwnerInfo = async (
  ...
  try {
    const nToken = runtime.provider.connectMultiAbi(
    ...
    const nftOwners =
      (
        await Promise.all(
          chunk(calls, 2000)
            .map(batch => runtime.provider.getMulticallProvider().all(batch))
        )
      ).flat()
...
const getValidStakedTokens = async (): Promise<ValidTokens> => {
  ...
  const apeCoinStaking: Types.ApeCoinStaking =
    await runtime.provider.connectContract(ParaspaceMM.ApeCoinStaking);
...
const filterByUserLimit = async (validTokens: ValidTokens): Promise<ValidCompoundInfo> => {
  ...
  Array.from([
    validTokens.validBayc,
    validTokens.validMayc
  ]).forEach((BaycOrMayc, index) => {
